...
import { Plus, Edit2, Save, X, Settings, AlertCircle } from 'lucide-react';
...
const EquipmentManager: React.FC<EquipmentManagerProps> = ({ onClose }) => {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      setError(error.message || 'Ошибка при добавлении оборудования');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleUpdate = async (id: string, data: { name: string; description: string; is_active: boolean }) => {
    ...
    try {
      ...
      setError(error.message || 'Ошибка при обновлении оборудования');
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-semibold text-gray-900">Управление оборудованием</h2>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="space-y-4">
            {equipment.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-gray-600">Добавьте первое оборудование в справочник</p>
              </div>
            ) : (
              equipment.map(item => (
                <EquipmentItem
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={handleSave}
              disabled={saving}
              className="flex items-center gap-2 px-3 py-1 text-white bg-blue-600 rounded-md hover:bg-blue-700 transition-colors disabled:opacity-50"
            >
              {saving ? (
                <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>
              ) : (
